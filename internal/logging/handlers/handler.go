...
func BuildOptions(ctx context.Context, k8s client.Client, mcAddon *addonapiv1beta1.ManagedClusterAddOn, platform, userWorkloads addon.LogsOptions, isHub bool, hubHostname string) (manifests.Options, error) {
	...
	if err := buildUnmanagedOptions(ctx, k8s, mcAddon, &opts); err != nil {
		return opts, err
	}
