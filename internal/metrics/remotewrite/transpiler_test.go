...
func convertToPromRelabel(cfgs []cooprometheusv1.RelabelConfig) []*relabel.Config {
...
func TestTranspile(t *testing.T) {
				...
				RemoteWrite: []cooprometheusv1.RemoteWriteSpec{
					{
						...
						Authorization: &cooprometheusv1.Authorization{
							SafeAuthorization: cooprometheusv1.SafeAuthorization{
								Type: "Bearer",
							},
						},
					},
	...
	if got.RemoteTimeout == nil || *got.RemoteTimeout != cooprometheusv1.Duration("30s") {
...
func TestSharedPointerCacheIsolation(t *testing.T) {
	...
	originalAgent := &cooprometheusv1alpha1.PrometheusAgent{
		Spec: cooprometheusv1alpha1.PrometheusAgentSpec{
			CommonPrometheusFields: cooprometheusv1.CommonPrometheusFields{
				RemoteWrite: []cooprometheusv1.RemoteWriteSpec{
					{
	...
	*got.RemoteTimeout = cooprometheusv1.Duration("99s")
	...
	if *originalAgent.Spec.RemoteWrite[0].RemoteTimeout != cooprometheusv1.Duration("30s") {
...
func TestTranspile_PreservesIdentificationRelabelConfigs(t *testing.T) {
	...
	res := lb.Labels()
	if got := res.Get("cluster"); got != "local-cluster" {
		t.Errorf("Expected cluster label to be 'local-cluster', got %q", got)
	}
	if got := res.Get("clusterID"); got != "cc983d7a-2cd2-4171-b55c-d978e587a978" {
		t.Errorf("Expected clusterID label to be 'cc983d7a-2cd2-4171-b55c-d978e587a978', got %q", got)
	}
}
