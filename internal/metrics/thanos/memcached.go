...
func (b *ObjectBuilder) buildMemcachedObjects() []runtime.Object {
	storeDeployment := buildMemcachedDeployment(config.MemcachedStoreName, memcachedStoreLabels())
	...
	qfeDeployment := buildMemcachedDeployment(config.MemcachedQueryFrontendName, memcachedQueryFrontendLabels())
...
func buildMemcachedDeployment(name string, labels map[string]string) *appsv1.Deployment {
			...
			Template: corev1.PodTemplateSpec{
				...
				Spec: corev1.PodSpec{
					...
					Containers: []corev1.Container{
						{
							Name:  "memcached",
							Image: config.DefaultMemcachedImage,
							Args:  []string{memoryLimitArg, connLimitArg, maxItemArg},
						...
						{
							Name:  "exporter",
							Image: config.DefaultMemcachedExporterImage,
							Args:  []string{fmt.Sprintf("--memcached.address=localhost:%d", memcachedPort)},
							Ports: []corev1.ContainerPort{
								{
									Name:          "metrics",
