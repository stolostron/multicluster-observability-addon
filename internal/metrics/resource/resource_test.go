...
func TestGetOrCreateDefaultAgent(t *testing.T) {
	...
	existingPlatformAgent := NewDefaultPrometheusAgent(config.HubInstallNamespace, makeAgentName(platformAppName, placementName), false)
	assert.NoError(t, controllerutil.SetControllerReference(cmao, existingPlatformAgent, newTestScheme()))

	// Existing UWL Agent
	uwlAppName := "uwl-app"
	existingUWLAgent := NewDefaultPrometheusAgent(config.HubInstallNamespace, makeAgentName(uwlAppName, placementName), true)
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			gotAgent, err := d.getOrCreateDefaultAgent(context.Background(), tc.placementName, tc.appName, tc.isUWL)
...
func TestReconcileAgent(t *testing.T) {
	...
	// Dynamic fake client doesn't support apply types of patch. This is overriden with an interceptor toward a
...
func TestCleanOrphanResources(t *testing.T) {
	...
	platformAppName := "platform-app"
	platformAgent := NewDefaultPrometheusAgent(config.HubInstallNamespace, makeAgentName(platformAppName, placementRefA.Name), false)
