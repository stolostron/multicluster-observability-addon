		...
		panel.AddQuery(
			query.PromQL(
				promql.SetLabelMatchers(
					"(instance:node_cpu_utilisation:rate1m{cluster=\"$cluster\",job=\"node-exporter\"} * instance:node_num_cpu:sum{cluster=\"$cluster\",job=\"node-exporter\"}) / scalar(sum(instance:node_num_cpu:sum{cluster=\"$cluster\",job=\"node-exporter\"}))", //nolint:misspell // upstream metric name
					labelMatchers,
				),
...
func MemoryUtilization(datasourceName string, labelMatchers ...promql.LabelMatcher) panelgroup.Option {
	return panelgroup.AddPanel("Memory Utilization",
		...
		panel.AddQuery(
			query.PromQL(
				promql.SetLabelMatchers(
					"instance:node_memory_utilisation:ratio{cluster=\"$cluster\",job=\"node-exporter\"} / scalar(count(instance:node_memory_utilisation:ratio{cluster=\"$cluster\",job=\"node-exporter\"}))", //nolint:misspell // upstream metric name
					labelMatchers,
				),
				query.SeriesNameFormat("{{ instance }}"),
				dashboards.AddQueryDataSource(datasourceName),
