...
func VMTotalCPUOverestimationPanel(datasourceName string) panelgroup.Option {
	return BuildStatPanel(datasourceName, StatPanelConfig{
		...
		Query: `sum(floor((max_over_time(sum by (name, namespace) (acm_rs_vm:namespace:cpu_request{cluster="$cluster", namespace=~"$namespace"}` +
...
func VMTotalMemOverestimationPanel(datasourceName string) panelgroup.Option {
	return BuildStatPanel(datasourceName, StatPanelConfig{
		...
		Query: `sum(floor((max_over_time(sum by (name, namespace) (acm_rs_vm:namespace:memory_request{cluster="$cluster", namespace=~"$namespace"}` +
			"\n" + `+ on(cluster, namespace, name) group_left(_blah)(0 * max by (cluster, namespace, name) (kubevirt_vm_running_status_last_transition_timestamp_seconds{cluster="$cluster", namespace=~"$namespace"} > 0))` +
			"\n" + `)[$days:]))-` +
...
func VMMemOverestimationTablePanel(datasourceName string, project string) panelgroup.Option {
	...
	return panelgroup.AddPanel("Memory Overestimation",
		...
		panel.AddQuery(query.PromQL(`label_join(`+"\n"+`    (max_over_time(sum by (name, namespace) (acm_rs_vm:namespace:memory_request{cluster="$cluster", profile="$profile", namespace=~"$namespace"})[$days:])),`+"\n"+`    "name_namespace", "-", "name", "namespace"`+"\n"+`)`, dashboards.AddQueryDataSource(datasourceName))),
		panel.AddQuery(query.PromQL(`label_join(`+"\n"+`    (max_over_time(sum by (name, namespace) (acm_rs_vm:namespace:memory_recommendation{cluster="$cluster", profile="$profile", namespace=~"$namespace"})[$days:])),`+"\n"+`    "name_namespace", "-", "name", "namespace"`+"\n"+`)`, dashboards.AddQueryDataSource(datasourceName))),
		panel.AddQuery(query.PromQL(`label_join(`+"\n"+`    ((max_over_time(sum by (name, namespace) (acm_rs_vm:namespace:memory_request{cluster="$cluster", profile="$profile", namespace=~"$namespace"})[$days:])) - (max_over_time(sum by (name, namespace) (acm_rs_vm:namespace:memory_recommendation{cluster="$cluster", profile="$profile", namespace=~"$namespace"})[$days:]))),`+"\n"+`    "name_namespace", "-", "name", "namespace"`+"\n"+`)`, dashboards.AddQueryDataSource(datasourceName))),
	)
}
...
func VMMemUnderestimationTablePanel(datasourceName string, project string) panelgroup.Option {
	...
	return panelgroup.AddPanel("Memory Underestimation",
		...
		panel.AddQuery(query.PromQL(`label_join(`+"\n"+`    (max_over_time(sum by (name, namespace) (acm_rs_vm:namespace:memory_request{cluster="$cluster", profile="$profile", namespace=~"$namespace"})[$days:])),`+"\n"+`    "name_namespace", "-", "name", "namespace"`+"\n"+`)`, dashboards.AddQueryDataSource(datasourceName))),
		panel.AddQuery(query.PromQL(`label_join(`+"\n"+`    (max_over_time(sum by (name, namespace) (acm_rs_vm:namespace:memory_recommendation{cluster="$cluster", profile="$profile", namespace=~"$namespace"})[$days:])),`+"\n"+`    "name_namespace", "-", "name", "namespace"`+"\n"+`)`, dashboards.AddQueryDataSource(datasourceName))),
		panel.AddQuery(query.PromQL(`label_join(`+"\n"+`    ((max_over_time(sum by (name, namespace) (acm_rs_vm:namespace:memory_request{cluster="$cluster", profile="$profile", namespace=~"$namespace"})[$days:])) - (max_over_time(sum by (name, namespace) (acm_rs_vm:namespace:memory_recommendation{cluster="$cluster", profile="$profile", namespace=~"$namespace"})[$days:]))) * (-1),`+"\n"+`    "name_namespace", "-", "name", "namespace"`+"\n"+`)`, dashboards.AddQueryDataSource(datasourceName))),
	)
}
...
func VMCPUOverestimationStatPanel(datasourceName string) panelgroup.Option {
	return BuildStatPanel(datasourceName, StatPanelConfig{
		...
		Query:       `floor(max_over_time(acm_rs_vm:namespace:cpu_request{cluster="$cluster", profile="$profile", namespace=~"$namespace", name=~"$vm"}[$days:])-` + "\n" + `max_over_time(acm_rs_vm:namespace:cpu_recommendation{cluster="$cluster", profile="$profile", namespace=~"$namespace", name=~"$vm"}[$days:]))`,
		Unit:        &dashboards.DecimalUnit,
		Decimals:    0,
		FontSize:    48,
...
func VMCPUUsageStatPanel(datasourceName string) panelgroup.Option {
	return BuildStatPanel(datasourceName, StatPanelConfig{
		...
		Query:       `max_over_time(acm_rs_vm:namespace:cpu_usage{cluster="$cluster", profile="$profile", namespace=~"$namespace", name=~"$vm"}[$days:])`,
		Unit:        &dashboards.DecimalUnit,
		Decimals:    0,
		FontSize:    48,
...
func VMCPURequestStatPanel(datasourceName string) panelgroup.Option {
	return BuildStatPanel(datasourceName, StatPanelConfig{
		...
		Query:       `max_over_time(acm_rs_vm:namespace:cpu_request{cluster="$cluster", profile="$profile", namespace=~"$namespace", name=~"$vm"}[$days:])`,
		Unit:        &dashboards.DecimalUnit,
		Decimals:    0,
		FontSize:    48,
		Thresholds:  detailGrayThreshold,
...
func VMCPUUtilizationStatPanel(datasourceName string) panelgroup.Option {
	return BuildStatPanel(datasourceName, StatPanelConfig{
		...
		Query:       `max_over_time(acm_rs_vm:namespace:cpu_usage{cluster="$cluster", profile="$profile", namespace=~"$namespace", name=~"$vm"}[$days:]) / max_over_time(acm_rs_vm:namespace:cpu_request{cluster="$cluster", profile="$profile", namespace=~"$namespace", name=~"$vm"}[$days:])`,
...
func VMMemoryOverestimationStatPanel(datasourceName string) panelgroup.Option {
	return BuildStatPanel(datasourceName, StatPanelConfig{
		...
		Query:       `(max_over_time(acm_rs_vm:namespace:memory_request{cluster="$cluster", profile="$profile", namespace=~"$namespace", name=~"$vm"}[$days:])) -` + "\n" + `(max_over_time(acm_rs_vm:namespace:memory_recommendation{cluster="$cluster", profile="$profile", namespace=~"$namespace", name=~"$vm"}[$days:]))`,
		Unit:        &dashboards.BytesUnit,
		FontSize:    48,
...
func VMMemoryUsageStatPanel(datasourceName string) panelgroup.Option {
	return BuildStatPanel(datasourceName, StatPanelConfig{
		...
		Query:       `max_over_time(acm_rs_vm:namespace:memory_usage{cluster="$cluster", profile="$profile", namespace=~"$namespace", name=~"$vm"}[$days:])`,
		Unit:        &dashboards.BytesUnit,
		FontSize:    48,
		Thresholds:  detailGrayThreshold,
	})
...
func VMMemoryUtilizationStatPanel(datasourceName string) panelgroup.Option {
	return BuildStatPanel(datasourceName, StatPanelConfig{
		...
		Query:       `max_over_time(acm_rs_vm:namespace:memory_usage{cluster="$cluster", profile="$profile", namespace=~"$namespace", name=~"$vm"}[$days:]) / max_over_time(acm_rs_vm:namespace:memory_request{cluster="$cluster", profile="$profile", namespace=~"$namespace", name=~"$vm"}[$days:])`,
		Unit:        &dashboards.PercentDecimalUnit,
		FontSize:    48,
		Thresholds:  detailPercentThreshold,
	})
...
func VMMemoryUnderestimationStatPanel(datasourceName string) panelgroup.Option {
	return BuildStatPanel(datasourceName, StatPanelConfig{
		...
		Query:       `((max_over_time(acm_rs_vm:namespace:memory_request{cluster="$cluster", profile="$profile", namespace=~"$namespace", name=~"$vm"}[$days:])) -` + "\n" + `(max_over_time(acm_rs_vm:namespace:memory_recommendation{cluster="$cluster", profile="$profile", namespace=~"$namespace", name=~"$vm"}[$days:])))* (-1)`,
		Unit:        &dashboards.BytesUnit,
		FontSize:    48,
