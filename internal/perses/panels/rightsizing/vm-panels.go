...
func vmDataLink(project, targetDashboard, title string) *DataLink {
	return &DataLink{
		...
		Title:      title,
		URL: fmt.Sprintf(
			"/monitoring/v2/dashboards/view?dashboard=%s&project=%s&var-cluster=${__data.fields[\"cluster\"]}&var-namespace=${__data.fields[\"namespace\"]}&var-vm=${__data.fields[\"name\"]}&var-days=$days&var-cpu_profile=$cpu_profile&var-memory_profile=$memory_profile&start=$__range",
...
const cpuOverestCond = `(floor(max_over_time(sum by (name, namespace) (acm_rs_vm:namespace:cpu_request{cluster="$cluster", profile="$cpu_profile", namespace=~"$namespace"})[$days:])-` +
...
const cpuUnderestCond = `(floor(max_over_time(sum by (name, namespace) (acm_rs_vm:namespace:cpu_request{cluster="$cluster", profile="$cpu_profile", namespace=~"$namespace"})[$days:])-` +
	`max_over_time(sum by (name, namespace) (acm_rs_vm:namespace:cpu_recommendation{cluster="$cluster", profile="$cpu_profile", namespace=~"$namespace"})[$days:])) < 0)`

const memOverestCond = `(floor((max_over_time(sum by (name, namespace) (acm_rs_vm:namespace:memory_request{cluster="$cluster", profile="$memory_profile", namespace=~"$namespace"})[$days:])/1073741824) - ` +
...
const memUnderestCond = `(floor((max_over_time(sum by (name, namespace) (acm_rs_vm:namespace:memory_request{cluster="$cluster", profile="$memory_profile", namespace=~"$namespace"})[$days:])/1073741824) - ` +
	`(max_over_time(sum by (name, namespace) (acm_rs_vm:namespace:memory_recommendation{cluster="$cluster", profile="$memory_profile", namespace=~"$namespace"})[$days:])/1073741824)) < 0)`

// runningVMFilter checks if VM is CURRENTLY running (instant query).
...
func VMCPUOverestimationTablePanel(datasourceName string, project string) panelgroup.Option {
	...
	return panelgroup.AddPanel("CPU Overestimation",
		...
		panel.AddQuery(query.PromQL(`label_join(((max_over_time(sum by (name, namespace) (acm_rs_vm:namespace:cpu_usage{cluster="$cluster", profile="$cpu_profile", namespace=~"$namespace"})[$days:]) / max_over_time(sum by (name, namespace) (acm_rs_vm:namespace:cpu_request{cluster="$cluster", profile="$cpu_profile", namespace=~"$namespace"})[$days:])) > 0) and on (name, namespace) `+cpuOverestCond+` and on (name, namespace) `+runningVMFilter+`, "name_namespace", "-", "name", "namespace")`, dashboards.AddQueryDataSource(datasourceName))),
		panel.AddQuery(query.PromQL(`label_join(max_over_time(sum by (name, namespace) (acm_rs_vm:namespace:cpu_usage{cluster="$cluster", profile="$cpu_profile", namespace=~"$namespace"})[$days:]) and on (name, namespace) `+cpuOverestCond+` and on (name, namespace) `+runningVMFilter+`, "name_namespace", "-", "name", "namespace")`, dashboards.AddQueryDataSource(datasourceName))),
		panel.AddQuery(query.PromQL(`label_join(max_over_time(sum by (name, namespace) (acm_rs_vm:namespace:cpu_request{cluster="$cluster", profile="$cpu_profile", namespace=~"$namespace"})[$days:]) and on (name, namespace) `+cpuOverestCond+` and on (name, namespace) `+runningVMFilter+`, "name_namespace", "-", "name", "namespace")`, dashboards.AddQueryDataSource(datasourceName))),
		panel.AddQuery(query.PromQL(`label_join(ceil(max_over_time(sum by (name, namespace) (acm_rs_vm:namespace:cpu_recommendation{cluster="$cluster", profile="$cpu_profile", namespace=~"$namespace"})[$days:])) and on (name, namespace) `+cpuOverestCond+` and on (name, namespace) `+runningVMFilter+`, "name_namespace", "-", "name", "namespace")`, dashboards.AddQueryDataSource(datasourceName))),
		panel.AddQuery(query.PromQL(`label_join(floor(max_over_time(sum by (name, namespace) (acm_rs_vm:namespace:cpu_request{cluster="$cluster", profile="$cpu_profile", namespace=~"$namespace"})[$days:])-max_over_time(sum by (name, namespace) (acm_rs_vm:namespace:cpu_recommendation{cluster="$cluster", profile="$cpu_profile", namespace=~"$namespace"})[$days:]))> 0 and on (name, namespace) `+runningVMFilter+`, "name_namespace", "-", "name", "namespace")`, dashboards.AddQueryDataSource(datasourceName))),
	)
}
...
func VMCPUUnderestimationTablePanel(datasourceName string, project string) panelgroup.Option {
	...
	return panelgroup.AddPanel("CPU Underestimation",
		...
		panel.AddQuery(query.PromQL(`label_join(((max_over_time(sum by (name, namespace) (acm_rs_vm:namespace:cpu_usage{cluster="$cluster", profile="$cpu_profile", namespace=~"$namespace"})[$days:]) / max_over_time(sum by (name, namespace) (acm_rs_vm:namespace:cpu_request{cluster="$cluster", profile="$cpu_profile", namespace=~"$namespace"})[$days:])) > 0) and on (name, namespace) `+cpuUnderestCond+` and on (name, namespace) `+runningVMFilter+`, "name_namespace", "-", "name", "namespace")`, dashboards.AddQueryDataSource(datasourceName))),
		panel.AddQuery(query.PromQL(`label_join(max_over_time(sum by (name, namespace) (acm_rs_vm:namespace:cpu_usage{cluster="$cluster", profile="$cpu_profile", namespace=~"$namespace"})[$days:]) and on (name, namespace) `+cpuUnderestCond+` and on (name, namespace) `+runningVMFilter+`, "name_namespace", "-", "name", "namespace")`, dashboards.AddQueryDataSource(datasourceName))),
		panel.AddQuery(query.PromQL(`label_join(max_over_time(sum by (name, namespace) (acm_rs_vm:namespace:cpu_request{cluster="$cluster", profile="$cpu_profile", namespace=~"$namespace"})[$days:]) and on (name, namespace) `+cpuUnderestCond+` and on (name, namespace) `+runningVMFilter+`, "name_namespace", "-", "name", "namespace")`, dashboards.AddQueryDataSource(datasourceName))),
		panel.AddQuery(query.PromQL(`label_join(ceil(max_over_time(sum by (name, namespace) (acm_rs_vm:namespace:cpu_recommendation{cluster="$cluster", profile="$cpu_profile", namespace=~"$namespace"})[$days:])) and on (name, namespace) `+cpuUnderestCond+` and on (name, namespace) `+runningVMFilter+`, "name_namespace", "-", "name", "namespace")`, dashboards.AddQueryDataSource(datasourceName))),
		panel.AddQuery(query.PromQL(`label_join((floor(max_over_time(sum by (name, namespace) (acm_rs_vm:namespace:cpu_request{cluster="$cluster", profile="$cpu_profile", namespace=~"$namespace"})[$days:])-max_over_time(sum by (name, namespace) (acm_rs_vm:namespace:cpu_recommendation{cluster="$cluster", profile="$cpu_profile", namespace=~"$namespace"})[$days:])) * (-1)) > 0 and on (name, namespace) `+runningVMFilter+`, "name_namespace", "-", "name", "namespace")`, dashboards.AddQueryDataSource(datasourceName))),
	)
}
...
func VMMemOverestimationTablePanel(datasourceName string, project string) panelgroup.Option {
	...
	return panelgroup.AddPanel("Memory Overestimation",
		...
		panel.AddQuery(query.PromQL(`label_join((max_over_time(sum by (name, namespace) (acm_rs_vm:namespace:memory_usage{cluster="$cluster", profile="$memory_profile", namespace=~"$namespace"})[$days:]) / max_over_time(sum by (name, namespace) (acm_rs_vm:namespace:memory_request{cluster="$cluster", profile="$memory_profile", namespace=~"$namespace"})[$days:])) and on (name, namespace) `+memOverestCond+` and on (name, namespace) `+runningVMFilter+`, "name_namespace", "-", "name", "namespace")`, dashboards.AddQueryDataSource(datasourceName))),
		panel.AddQuery(query.PromQL(`label_join((max_over_time(sum by (name, namespace) (acm_rs_vm:namespace:memory_usage{cluster="$cluster", profile="$memory_profile", namespace=~"$namespace"})[$days:]) /1073741824) * 1073741824 and on (name, namespace) `+memOverestCond+` and on (name, namespace) `+runningVMFilter+`, "name_namespace", "-", "name", "namespace")`, dashboards.AddQueryDataSource(datasourceName))),
		panel.AddQuery(query.PromQL(`label_join((max_over_time(sum by (name, namespace) (acm_rs_vm:namespace:memory_request{cluster="$cluster", profile="$memory_profile", namespace=~"$namespace"})[$days:])/1073741824) * 1073741824 and on (name, namespace) `+memOverestCond+` and on (name, namespace) `+runningVMFilter+`, "name_namespace", "-", "name", "namespace")`, dashboards.AddQueryDataSource(datasourceName))),
		panel.AddQuery(query.PromQL(`label_join((ceil(max_over_time(sum by (name, namespace) (acm_rs_vm:namespace:memory_recommendation{cluster="$cluster", profile="$memory_profile", namespace=~"$namespace"})[$days:])/ 1073741824)) * 1073741824 and on (name, namespace) `+memOverestCond+` and on (name, namespace) `+runningVMFilter+`, "name_namespace", "-", "name", "namespace")`, dashboards.AddQueryDataSource(datasourceName))),
		panel.AddQuery(query.PromQL(`label_join((floor((max_over_time(sum by (name, namespace) (acm_rs_vm:namespace:memory_request{cluster="$cluster", profile="$memory_profile", namespace=~"$namespace"})[$days:])) / 1073741824 - (max_over_time(sum by (name, namespace) (acm_rs_vm:namespace:memory_recommendation{cluster="$cluster", profile="$memory_profile", namespace=~"$namespace"})[$days:])) / 1073741824)) * 1073741824 > 0 and on (name, namespace) `+runningVMFilter+`, "name_namespace", "-", "name", "namespace")`, dashboards.AddQueryDataSource(datasourceName))),
	)
}
...
func VMMemUnderestimationTablePanel(datasourceName string, project string) panelgroup.Option {
	...
	return panelgroup.AddPanel("Memory Underestimation",
		...
		panel.AddQuery(query.PromQL(`label_join((max_over_time(sum by (name, namespace) (acm_rs_vm:namespace:memory_usage{cluster="$cluster", profile="$memory_profile", namespace=~"$namespace"})[$days:]) / max_over_time(sum by (name, namespace) (acm_rs_vm:namespace:memory_request{cluster="$cluster", profile="$memory_profile", namespace=~"$namespace"})[$days:])) and on (name, namespace) `+memUnderestCond+` and on (name, namespace) `+runningVMFilter+`, "name_namespace", "-", "name", "namespace")`, dashboards.AddQueryDataSource(datasourceName))),
		panel.AddQuery(query.PromQL(`label_join((max_over_time(sum by (name, namespace) (acm_rs_vm:namespace:memory_usage{cluster="$cluster", profile="$memory_profile", namespace=~"$namespace"})[$days:])/1073741824) * 1073741824 and on (name, namespace) `+memUnderestCond+` and on (name, namespace) `+runningVMFilter+`, "name_namespace", "-", "name", "namespace")`, dashboards.AddQueryDataSource(datasourceName))),
		panel.AddQuery(query.PromQL(`label_join((max_over_time(sum by (name, namespace) (acm_rs_vm:namespace:memory_request{cluster="$cluster", profile="$memory_profile", namespace=~"$namespace"})[$days:])/1073741824) * 1073741824 and on (name, namespace) `+memUnderestCond+` and on (name, namespace) `+runningVMFilter+`, "name_namespace", "-", "name", "namespace")`, dashboards.AddQueryDataSource(datasourceName))),
		panel.AddQuery(query.PromQL(`label_join((ceil(max_over_time(sum by (name, namespace) (acm_rs_vm:namespace:memory_recommendation{cluster="$cluster", profile="$memory_profile", namespace=~"$namespace"})[$days:])/1073741824)) * 1073741824 and on (name, namespace) `+memUnderestCond+` and on (name, namespace) `+runningVMFilter+`, "name_namespace", "-", "name", "namespace")`, dashboards.AddQueryDataSource(datasourceName))),
		panel.AddQuery(query.PromQL(`label_join(((floor((max_over_time(sum by (name, namespace) (acm_rs_vm:namespace:memory_request{cluster="$cluster", profile="$memory_profile", namespace=~"$namespace"})[$days:])) / 1073741824 - (max_over_time(sum by (name, namespace) (acm_rs_vm:namespace:memory_recommendation{cluster="$cluster", profile="$memory_profile", namespace=~"$namespace"})[$days:])) / 1073741824)) * (-1)) * 1073741824 > 0 and on (name, namespace) `+runningVMFilter+`, "name_namespace", "-", "name", "namespace")`, dashboards.AddQueryDataSource(datasourceName))),
	)
}
...
func VMCPUOverestimationStatPanel(datasourceName string) panelgroup.Option {
	return BuildStatPanel(datasourceName, StatPanelConfig{
		...
		Query:       `max by (cluster, profile, namespace, name)(floor(max_over_time(acm_rs_vm:namespace:cpu_request{cluster="$cluster", profile="$cpu_profile", namespace=~"$namespace", name=~"$vm"}[$days:])-` + "\n" + `max_over_time(acm_rs_vm:namespace:cpu_recommendation{cluster="$cluster", profile="$cpu_profile", namespace=~"$namespace", name=~"$vm"}[$days:])))`,
		Unit:        &dashboards.DecimalUnit,
		Decimals:    0,
		FontSize:    40,
...
func VMCPUUsageStatPanel(datasourceName string) panelgroup.Option {
	return BuildStatPanel(datasourceName, StatPanelConfig{
		...
		Query:       `max by (cluster, profile, namespace, name)(max_over_time(acm_rs_vm:namespace:cpu_usage{cluster="$cluster", profile="$cpu_profile", namespace=~"$namespace", name=~"$vm"}[$days:]))`,
		Unit:        &dashboards.DecimalUnit,
		Decimals:    0,
		FontSize:    40,
...
func VMCPURequestStatPanel(datasourceName string) panelgroup.Option {
	return BuildStatPanel(datasourceName, StatPanelConfig{
		...
		Query:       `max by (cluster, profile, namespace, name)(max_over_time(acm_rs_vm:namespace:cpu_request{cluster="$cluster", profile="$cpu_profile", namespace=~"$namespace", name=~"$vm"}[$days:]))`,
		Unit:        &dashboards.DecimalUnit,
		Decimals:    0,
		FontSize:    40,
		Thresholds:  detailGrayThreshold,
...
func VMCPUUtilizationStatPanel(datasourceName string) panelgroup.Option {
	return BuildStatPanel(datasourceName, StatPanelConfig{
		...
		Query:       `max by (cluster, profile, namespace, name)(max_over_time(acm_rs_vm:namespace:cpu_usage{cluster="$cluster", profile="$cpu_profile", namespace=~"$namespace", name=~"$vm"}[$days:]) / max_over_time(acm_rs_vm:namespace:cpu_request{cluster="$cluster", profile="$cpu_profile", namespace=~"$namespace", name=~"$vm"}[$days:]))`,
...
func VMMemoryOverestimationStatPanel(datasourceName string) panelgroup.Option {
	return BuildStatPanel(datasourceName, StatPanelConfig{
		...
		Query:       `max by (cluster, profile, namespace, name)((floor((max_over_time(acm_rs_vm:namespace:memory_request{cluster="$cluster", profile="$memory_profile", namespace=~"$namespace", name=~"$vm"}[$days:])/ 1073741824) -` + "\n" + `(max_over_time(acm_rs_vm:namespace:memory_recommendation{cluster="$cluster", profile="$memory_profile", namespace=~"$namespace", name=~"$vm"}[$days:])/ 1073741824))) * 1073741824)`,
		Unit:        &dashboards.BytesUnit,
		FontSize:    40,
...
func VMMemoryUsageStatPanel(datasourceName string) panelgroup.Option {
	return BuildStatPanel(datasourceName, StatPanelConfig{
		...
		Query:       `max by (cluster, profile, namespace, name)((max_over_time(acm_rs_vm:namespace:memory_usage{cluster="$cluster", profile="$memory_profile", namespace=~"$namespace", name=~"$vm"}[$days:])/ 1073741824) * 1073741824)`,
		Unit:        &dashboards.BytesUnit,
		FontSize:    40,
		Thresholds:  detailGrayThreshold,
	})
...
func VMMemoryRequestStatPanel(datasourceName string) panelgroup.Option {
	return BuildStatPanel(datasourceName, StatPanelConfig{
		...
		Query:       `max by (cluster, profile, namespace, name)((max_over_time(acm_rs_vm:namespace:memory_request{cluster="$cluster", profile="$memory_profile", namespace=~"$namespace", name=~"$vm"}[$days:])/ 1073741824) * 1073741824)`,
		Unit:        &dashboards.BytesUnit,
		FontSize:    40,
		Thresholds:  detailGrayThreshold,
	})
...
func VMMemoryUtilizationStatPanel(datasourceName string) panelgroup.Option {
	return BuildStatPanel(datasourceName, StatPanelConfig{
		...
		Query:       `max by (cluster, profile, namespace, name)(max_over_time(acm_rs_vm:namespace:memory_usage{cluster="$cluster", profile="$memory_profile", namespace=~"$namespace", name=~"$vm"}[$days:]) / max_over_time(acm_rs_vm:namespace:memory_request{cluster="$cluster", profile="$memory_profile", namespace=~"$namespace", name=~"$vm"}[$days:]))`,
		Unit:        &dashboards.PercentDecimalUnit,
		FontSize:    40,
		Thresholds:  detailPercentThreshold,
	})
...
func VMMemoryUnderestimationStatPanel(datasourceName string) panelgroup.Option {
	return BuildStatPanel(datasourceName, StatPanelConfig{
		...
		Query:       `max by (cluster, profile, namespace, name)((floor((max_over_time(acm_rs_vm:namespace:memory_request{cluster="$cluster", profile="$memory_profile", namespace=~"$namespace", name=~"$vm"}[$days:])/ 1073741824) -` + "\n" + `(max_over_time(acm_rs_vm:namespace:memory_recommendation{cluster="$cluster", profile="$memory_profile", namespace=~"$namespace", name=~"$vm"}[$days:])/ 1073741824))* (-1)) * 1073741824)`,
		Unit:        &dashboards.BytesUnit,
		FontSize:    40,
